 ...
 * @author sean
 */
public class SummaryDataWriter implements DataWriter {

	private static class CsvSummaryExportFormatter {
	...
	private CsvSummaryExportFormatter formatter = new CsvSummaryExportFormatter();

	public SummaryDataWriter() {
	...
	public void exportSummaryFromModel(GCModel model, String filePath) throws IOException {
		try (FileWriter fileWriter = new FileWriter(filePath)) {
			...
			int lastIndexOfSlash = model.getURL().getFile().lastIndexOf('/');
			exportValue(printWriter, "gcLogFile",
					model.getURL().getFile().substring(lastIndexOfSlash >= 0 ? lastIndexOfSlash + 1 : 0), "-");

			exportMemorySummary(printWriter, model);
			exportPauseSummary(printWriter, model);
			exportOverallSummary(printWriter, model);
		}
	}
	...
	private void exportOverallSummary(PrintWriter out, GCModel model) {
		...
		if (model.hasCorrectTimestamp()) {
			...
			formed = freedMemoryPerMinFormatter
					.formatToFormatted(model.getFreedMemory() / model.getRunningTime() * 60.0);
		...
		if (gcDataAvailable) {
			formed = footprintFormatter
					.formatToFormatted(model.getFreedMemoryByGC().getSum() / model.getGCPause().getSum());
		...
		if (fullGCDataAvailable) {
			formed = footprintFormatter
					.formatToFormatted(model.getFreedMemoryByFullGC().getSum() / model.getFullGCPause().getSum());
	...
	private void exportPauseSummary(PrintWriter out, GCModel model) {
		...
		if (pauseDataAvailable) {
			...
			exportValue(out, "avgPauseIsSig",
					isSignificant(model.getPause().average(), model.getPause().standardDeviation()));
			...
			if (gcDataAvailable) {
				...
				exportValue(out, "avgGCPauseIsSig",
						isSignificant(model.getGCPause().average(), model.getGCPause().standardDeviation()));
				exportValue(out, "avgGCPause", pauseFormatter.format(model.getGCPause().average()), "s");
				exportValue(out, "avgGCPause\u03c3", pauseFormatter.format(model.getGCPause().standardDeviation()),
						"s");
			} else {
			...
			if (fullGCDataAvailable) {
				...
				exportValue(out, "avgFullGCPauseIsSig",
						isSignificant(model.getFullGCPause().average(), model.getFullGCPause().standardDeviation()));
				exportValue(out, "avgFullGCPause", pauseFormatter.format(model.getFullGCPause().average()), "s");
				exportValue(out, "avgFullGCPause\u03c3",
						pauseFormatter.format(model.getFullGCPause().standardDeviation()), "s");
		...
		exportValue(out, "fullGCPausePc",
				percentFormatter.format(model.getFullGCPause().getSum() * 100.0 / model.getPause().getSum()), "%");
		exportValue(out, "gcPause", gcTimeFormatter.format(model.getGCPause().getSum()), "s");
		exportValue(out, "gcPausePc",
				percentFormatter.format(model.getGCPause().getSum() * 100.0 / model.getPause().getSum()), "%");
	}
	...
	private boolean isSignificant(double average, double standardDeviation) {
		...
		// Note: this may or may not be a good measure, but it at least helps to mark
		// some bad data as such
	...
	private void exportMemorySummary(PrintWriter out, GCModel model) {
		...
		exportValue(out, "totalHeapUsedMaxpc", percentFormatter
				.format(model.getHeapUsedSizes().getMax() * 100.0 / model.getHeapAllocatedSizes().getMax()), "%");
		...
		if (model.getTenuredAllocatedSizes().getN() == 0) {
			...
			exportValue(out, "totalTenuredUsedMaxpc",
					percentFormatter.format(
							model.getTenuredUsedSizes().getMax() * 100.0 / model.getTenuredAllocatedSizes().getMax()),
					"%");
		}
		...
		if (model.getYoungAllocatedSizes().getN() == 0) {
			...
			exportValue(out, "totalYoungUsedMaxpc", percentFormatter
					.format(model.getYoungUsedSizes().getMax() * 100.0 / model.getYoungAllocatedSizes().getMax()), "%");
		}
		...
		if (model.getPermAllocatedSizes().getN() == 0) {
			...
			exportValue(out, "totalPermUsedMaxpc", percentFormatter
					.format(model.getPermUsedSizes().getMax() * 100.0 / model.getPermAllocatedSizes().getMax()), "%");
		}
		...
		if (!fullGCDataVailable) {
			...
			exportValue(out, "freedMemoryByFullGCpc",
		...
		if (!gcDataAvailable) {
			...
			exportValue(out, "freedMemoryByGCpc",
		...
		boolean promotionDataAvailable = model.getPromotion().getN() != 0;
