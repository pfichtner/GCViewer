...
public class GCModel implements Serializable {
	...
	private List<AbstractGCEvent<?>> allEvents = new ArrayList<AbstractGCEvent<?>>();
	private List<AbstractGCEvent<?>> allEvents_ = unmodifiableList(allEvents);
	private List<AbstractGCEvent<?>> stopTheWorldEvents = new ArrayList<AbstractGCEvent<?>>();
	private List<AbstractGCEvent<?>> stopTheWorldEvents_ = unmodifiableList(stopTheWorldEvents);
	private List<GCEvent> gcEvents = new ArrayList<GCEvent>();
	private List<GCEvent> gcEvents_ = unmodifiableList(gcEvents);
	private List<AbstractGCEvent<?>> gcPhases = new ArrayList<AbstractGCEvent<?>>();
	private List<AbstractGCEvent<?>> vmOperationEvents = new ArrayList<AbstractGCEvent<?>>();
	private List<AbstractGCEvent<?>> vmOperationEvents_ = unmodifiableList(vmOperationEvents);
	private List<ConcurrentGCEvent> concurrentGCEvents = new ArrayList<ConcurrentGCEvent>();
	private List<ConcurrentGCEvent> concurrentGCEvents_ = unmodifiableList(concurrentGCEvents);
	private List<GCEvent> currentNoFullGCEvents = new ArrayList<GCEvent>();
	private List<GCEvent> fullGCEvents = new ArrayList<GCEvent>();
	private List<GCEvent> fullGCEvents_ = unmodifiableList(fullGCEvents);
	private FileInformation fileInformation = new FileInformation();

	private Map<String, DoubleData> fullGcEventPauses = new TreeMap<String, DoubleData>(); // pause information about
																							// all full gc events for
																							// detailed output
	private Map<String, DoubleData> gcEventPauses = new TreeMap<String, DoubleData>(); // pause information about all
																						// stw events for detailed
																						// output
	private Map<String, DoubleData> gcEventPhases = new TreeMap<String, DoubleData>(); // pause information about all
																						// phases for garbage collection
																						// events
	private Map<String, DoubleData> concurrentGcEventPauses = new TreeMap<String, DoubleData>(); // pause information
																									// about all
																									// concurrent events
	private Map<String, DoubleData> vmOperationEventPauses = new TreeMap<String, DoubleData>(); // pause information
																								// about vm operations
																								// ("application
																								// stopped")

	private IntData heapAllocatedSizes = new IntData(); // allocated heap size of every event
	private IntData tenuredAllocatedSizes = new IntData(); // allocated tenured size of every event that has this
															// information
	private IntData youngAllocatedSizes = new IntData(); // allocated young size of every event that has this
															// information
	private IntData permAllocatedSizes = new IntData(); // allocated perm size of every event that has this information
	private IntData heapUsedSizes = new IntData(); // used heap of every event
	private IntData tenuredUsedSizes = new IntData(); // used tenured size of every event that has this information
	private IntData youngUsedSizes = new IntData(); // used young size of every event that has this information
	private IntData permUsedSizes = new IntData(); // used perm size of every event that has this information

	private IntData postConcurrentCycleUsedTenuredSizes = new IntData(); // used tenured heap after concurrent
																			// collections
	private IntData postConcurrentCycleUsedHeapSizes = new IntData(); // used heap after concurrent collections

	private IntData promotion = new IntData(); // promotion from young to tenured generation during young collections
	...
	private double lastFullGcPauseTimeStamp;
	private DoubleData fullGcPauseInterval = new DoubleData(); // interval between two stop the Full GC pauses
	private DoubleData gcPause = new DoubleData(); // not full gc but stop the world pause
	private DoubleData vmOperationPause = new DoubleData(); // "application stopped"
	private double lastGcPauseTimeStamp = 0;
	private DoubleData pauseInterval = new DoubleData(); // interval between two stop the world pauses
	private DoubleData initiatingOccupancyFraction = new DoubleData(); // all concurrent collectors; start of
																		// concurrent collection
	private long freedMemory;
	private Format format;
	private IntData postGCUsedMemory = new IntData();
	private IntData postFullGCUsedHeap = new IntData();
	...
	private static class FileInformation implements Serializable {
		...
		private long determineCreationDate(File file, Optional<BasicFileAttributes> fileAttributes) {
			...
			return fileAttributes.isPresent() ? fileAttributes.get().creationTime().toMillis() : file.lastModified();
		}
		...
		public boolean equals(Object other) {
			...
			return fileInfo.lastModified == lastModified && fileInfo.creationTime == creationTime
	...
	private void printPauseMap(Map<String, DoubleData> pauseMap) {
		for (Map.Entry<String, DoubleData> entry : pauseMap.entrySet()) {
			System.out.println(entry.getKey() + " [n, avg, sum, min, max]:\t" + entry.getValue().getN() + "\t"
					+ entry.getValue().average() + "\t" + entry.getValue().getSum() + "\t" + entry.getValue().getMin()
	...
	private void printIntData(String name, IntData data) {
		try {
			System.out.println(name + " (n, avg, stddev, min, max):\t" + data.getN() + "\t" + data.average() + "\t"
					+ data.standardDeviation() + "\t" + data.getMin() + "\t" + data.getMax());
	...
	/**
	 * Returns the event that was last added or <code>null</code> if there is none
	 * yet.
	 *
	...
	/**
	 * Returns all stop the world events (everything that stops the vm to perfom its
	 * action - includes vm operations of present).
	 *
	 * @return all stop the world events
	 */
	public List<AbstractGCEvent<?>> getStopTheWorldEvents() {
		return stopTheWorldEvents_;
	}

	/**
	 * Returns all garbage collection events (without full gcs / vm operations).
	 *
	 * @return all gc events (without full gcs).
	 */
	...
	/**
	 * Returns all vm operation events.
	 *
	 * @return all vm operation events
	 */
	public List<AbstractGCEvent<?>> getVmOperationsEvents() {
		return vmOperationEvents_;
	}

	/**
	 * Returns to all concurrent gc events.
	 *
	 * @return all concurrent gc events.
	 */
	public List<ConcurrentGCEvent> getConcurrentGCEvents() {
		return concurrentGCEvents_;
	}

	/**
	 * Returns all events in the order they were added to the model.
	 *
	 * @return all events
	 */
	public List<AbstractGCEvent<?>> getEvents() {
		return allEvents_;
	}

	/**
	 * Returns all full gc events.
	 *
	 * @return all full gc events
	 */
	public List<GCEvent> getFullGCEvents() {
		return fullGCEvents_;
	}
	...
	public void add(AbstractGCEvent<?> abstractEvent) {
		...
		if (abstractEvent.isStopTheWorld()) {
			// totalPause must not be added here yet, because in case of vmOperationEvents,
			// the
		...
		if (size() == 1 || (size() > 1 && abstractEvent.getTimestamp() > 0.0)) {
			...
			// sometimes, no timestamp is present, because the line is mixed -> don't count
			// these here
		...
		if (abstractEvent.isStopTheWorld()) {
			// add to total pause here, because then adjusted VmOperationEvents are added
			// correctly
			// as well
	...
	private void addConcurrentGcEvent(ConcurrentGCEvent concEvent) {
		...
		if (concEvent.hasMemoryInformation() && concEvent.isConcurrentCollectionEnd()) {
			// register postConcurrentCycleUsedSizes, if event contains memory information.
			// Otherwise deduce it (see in handling of GCEvent)
	...
	private void addGcEvent(GCEvent event) {
		...
		if (size() > 1 && allEvents.get(allEvents.size() - 2).isConcurrentCollectionEnd()
				&& !allEvents.get(allEvents.size() - 2).hasMemoryInformation()) {
			// only deduce postConcurrentCycleUsedSizes, if concurrent event itself does not
			// contain memory information
	...
	private void addGcEventPause(GCEvent event) {
		// make a difference between stop the world events, which only collect from some
		// generations...
	...
	private void addFullGcEventPauses(GCEvent event) {
		...
		if (currentPostGCSlope.hasPoints()) {
			...
			if (currentPostGCSlope.isLine()) {
				...
				relativePostGCIncrease.add(currentRelativePostGCIncrease.slope(),
						currentRelativePostGCIncrease.getPointCount());
			}
	...
	private void addGcEventPhases(GCEvent event) {
		for (AbstractGCEvent<?> phaseEvent : event.getPhases()) {
			DoubleData phases = getDoubleData(phaseEvent.getTypeAsString(), gcEventPhases);
	...
	private void makeSureHasTimeStamp(AbstractGCEvent<?> abstractEvent) {
		if (size() >= 1 && abstractEvent.getTimestamp() < 0.000001 && abstractEvent.getDatestamp() != null) {
			// looks like there is no timestamp set -> set one, because a lot depends on the
			// timestamps
			abstractEvent.setTimestamp(
					ChronoUnit.MILLIS.between(getFirstDateStamp(), abstractEvent.getDatestamp()) / 1000.0);
		}
	}
	...
	private void updatePostConcurrentCycleUsedSizes(GCEvent event) {
		// Most interesting is the size of the life objects immediately after a
		// concurrent cycle.
		// Since the "concurrent-end" events don't have the heap size information, the
		// next event
		// after is taken to get the information. Young generation, that has already
		// filled up
		// again since the concurrent-end should not be counted, so take tenured size,
		// if available.
		...
		if (stopTheWorldEvents.size() > 1) {
			...
			if (!previousEvent.getExtendedType().getCollectionType().equals(CollectionType.VM_OPERATION)) {
				...
				if (adjustedPause > 0) {
					...
					// this happens if the first VM_OPERATION event after a GCEvent could not be
					// read (mixed with concurrent event)
	...
	/**
	 * Make sure, time / datestamp of <code>vmOpEvent</code> is at least as long
	 * later as the pause duration of <code>previousEvent</code>.
	 *
	...
	/**
	 * Promotion is the amount of memory that is promoted from young to tenured
	 * space during a collection of the young space.
	 *
	 * @param event
	 */
	private void updatePromotion(GCEvent event) {
		if (event.getGeneration().equals(Generation.YOUNG) && event.hasDetails() && !event.isFull()) {
			event.details().stream().filter(e -> e.getGeneration().equals(Generation.YOUNG)).findFirst()
					.ifPresent(e -> promotion
							.add((e.getPreUsed() - e.getPostUsed()) - (event.getPreUsed() - event.getPostUsed())));
		}
	}
	...
	private void updateInitiatingOccupancyFraction_(GCEvent event) {
		if (event.hasDetails()) {
			event.details().stream().filter(e -> e.isInitialMark()).findFirst().ifPresent(initialMarkEvent -> {
				...
				if (initialMarkEvent.getTotal() > 0) {
					initiatingOccupancyFraction
							.add(initialMarkEvent.getPreUsed() / (double) initialMarkEvent.getTotal());
				}
			});
		}
	...
	private void updateInitiatingOccupancyFraction(GCEvent event) {
		GCEvent initialMarkEvent = event.details().stream().filter(GCEvent::isInitialMark).findFirst().orElse(event);
	...
	/**
	 * @return Statistical data about pauses interval between full garbage
	 *         collections.
	 */
	...
	/**
	 * @return Statistical data about pauses caused by garbage collections (full gcs
	 *         excluded).
	 */
	...
	/**
	 * @return Statistical data about pauses caused by vm operations other than gc
	 *         pauses ("application stopped").
	 */
	...
	/**
	 * Return statistical data about fraction of tenured heap when concurrent
	 * collection cycles are started.
	 *
	 * @return statistical data about tenured heap occupation at start of concurrent
	 *         collections
	 */
	...
	/**
	 * @return The increase in memory consumption after a full collection in
	 *         relation to the amount that was used after the previous full
	 *         collection.
	 */
	...
	/**
	 * @return The increase in memory consumption after a collection in relation to
	 *         the amount that was used after the previous collection.
	 */
	...
	/**
	 * @return The average slope of the regression lines of the memory consumption
	 *         after a garbage collection in between <em>full</em> garbage
	 *         collections.
	 *         <p>
	 *         The unit is kb/s.
	 */
	...
	/**
	 * @return perm sizes allocated for every event that contained one (only if
	 *         detailed logging is active and and all spaces were collected)
	 */
	...
	/**
	 * @return tenured sizes allocated for every event that contained one (only if
	 *         detailed logging is active)
	 */
	...
	/**
	 * @return tenured sizes used for every event that contained one (only if
	 *         detailed logging is active)
	 */
	...
	/**
	 * @return young sizes allocated for every event that contained one (only if
	 *         detailed logging is active)
	 */
	...
	/**
	 * @return young sizes used for every event that contained one (only if detailed
	 *         logging is active)
	 */
	...
	/**
	 * @return Sizes of tenured heap (or if not available total heap) immediately
	 *         after completion of a concurrent cycle.
	 */
	...
	/**
	 * @return promotion information for all young collections (how much memory was
	 *         promoted to tenured space per young collection?)
	 */
	...
	public double getRunningTime() {
		return lastPauseTimeStamp - firstPauseTimeStamp
				+ (stopTheWorldEvents.size() > 0 ? stopTheWorldEvents.get(stopTheWorldEvents.size() - 1).getPause()
						: 0);
	}

	/**
	 * @return The timestamp of the first event in the log (which usually is
	 *         probably never exactly 0)
	 */
	...
	public boolean hasCorrectTimestamp() {
		return format == Format.IBM_VERBOSE_GC || format == Format.SUN_X_LOG_GC || format == Format.SUN_1_2_2VERBOSE_GC
				|| format == Format.UNIFIED_JVM_LOGGING;
	}

	public boolean hasDateStamp() {
		return allEvents.size() > 0 && allEvents.get(0).getDatestamp() != null;
	}

	public ZonedDateTime getFirstDateStamp() {
		return allEvents.size() > 0 ? get(0).getDatestamp() : null;
	}

	public Optional<Double> getFirstTimeStamp() {
		return allEvents.isEmpty() ? Optional.empty() : Optional.of(get(0).getTimestamp());
	}

	/**
	 * Best effort calculation of this {@link GCModel}s start date based on the
	 * available information
	 *
	 * @return the most probable start date of this {@link GCModel}
	 */
	public ZonedDateTime getStartDate() {
		ZonedDateTime suggestedStartDate = ZonedDateTime.ofInstant(Instant.ofEpochMilli(getLastModified()),
	...
	public int hashCode() {
		return Objects.hash(allEvents, fileInformation, fullGcEventPauses, gcEventPauses, gcEventPhases,
				concurrentGcEventPauses, vmOperationEventPauses, heapAllocatedSizes, tenuredAllocatedSizes,
				youngAllocatedSizes, permAllocatedSizes, heapUsedSizes, tenuredUsedSizes, youngUsedSizes, permUsedSizes,
				postConcurrentCycleUsedTenuredSizes, postConcurrentCycleUsedHeapSizes, promotion, firstPauseTimeStamp,
				lastPauseTimeStamp, totalPause, fullGCPause, lastFullGcPauseTimeStamp, fullGcPauseInterval, gcPause,
				vmOperationPause, lastGcPauseTimeStamp, pauseInterval, initiatingOccupancyFraction, freedMemory, format,
				postGCUsedMemory, postFullGCUsedHeap, freedMemoryByGC, freedMemoryByFullGC, postGCSlope,
				currentPostGCSlope, currentRelativePostGCIncrease, relativePostGCIncrease, postFullGCSlope,
				relativePostFullGCIncrease, url);
	}
