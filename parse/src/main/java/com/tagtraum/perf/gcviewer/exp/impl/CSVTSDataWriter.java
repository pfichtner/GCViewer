 ...
 * <p>
 * It uses the
 * {@literal "Timestamp(unix/#),Used(K),Total(K),Pause(sec),GC-Type"} format.
 *
...
public class CSVTSDataWriter implements DataWriter {
	...
	public void write(GCModel model, OutputStream outputstream, Map<String, Object> configuration) throws IOException {
		...
		model.getStopTheWorldEvents().stream().filter(GCEvent.class::isInstance).forEach(event -> {
			// Since this data writer is only concerned with one line per gc entry, don't
			// write two like the others.
